...
void make_initial_data(double *re_part, double *im_part, int num_of_data,
                       int power);
  ...
  for (i = 0; i < power; i++) {
    ...
    for (j = 0; j < num_of_dft; j++) {
      ...
      for (k = 0; k < dft; k++) {
        ...
        re_part_new[num_out] =
            *(re_part + num_in1) + re_buf * cos(angle) + im_buf * sin(angle);
        im_part_new[num_out] =
            *(im_part + num_in1) + im_buf * cos(angle) - re_buf * sin(angle);
        /* 角度を更新 */
