...
/* 原画像 image1[y][x] ===> 変換後の画像 image2[y][x]   */
{
  ...
  for (y = 1; y < y_size1 - 1; y++) {
    for (x = 1; x < x_size1 - 1; x++) {
      ...
      for (i = -1; i < 2; i++)
        for (j = -1; j < 2; j++)
          pixel_value =
              pixel_value + (weight[i + 1][j + 1] * image1[y + i][x + j]);
  ...
  y_size2 = y_size1;
  for (y = 0; y < y_size2; y++)
    for (x = 0; x < x_size2; x++) image2[y][x] = 0;
  /* 処理後の値を線形変換してから image2 に代入 */

  for (y = 1; y < y_size1 - 1; y++) {
    for (x = 1; x < x_size1 - 1; x++) {
      pixel_value = data[y][x];
      if (pixel_value > 0) pixel_value = 128 * pixel_value / max + 127;
      if (pixel_value < 0) pixel_value = 127 * (min - pixel_value) / min;
      if (pixel_value == 0) pixel_value = 127;
