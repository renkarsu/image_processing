/* pgm/ppm ファイルを読み書きするためのヘッダファイル mypgpm.h */
...
#define PRIMARY_COLOR 3    /* 光の原色数 */

/* PPM用の関数プロトタイプ宣言 */
void load_image_data_ppm(); /* 画像読み込み用関数 */
void save_image_data_ppm(); /* 画像書き込み用関数 */
...
int x_size1, y_size1; /* image1_ppm の横画素数，縦画素数 */
int x_size2, y_size2; /* image2_ppm の横画素数，縦画素数 */
...
/* image1[ ][ ]，x_size1，y_size1 にそれぞれ代入する       */
{
  ...
  if (buffer[0] != 'P' || buffer[1] != '6') {
    printf("ファイルのフォーマットが P6 とは異なります．\n");
    exit(1);
  }
  /* x_size1, y_size1 の代入（#から始まるコメントは読み飛ばす） */
  x_size1 = 0;
...
/* image2[ ][ ], x_size2, y_size2 のデータを，それぞれ ppm 画像，*/
/* 横画素数，縦画素数としてファイルに保存する．                  */
{
  char file_name[MAX_FILENAME]; /* ファイル名用の文字配列 */
  FILE *fp;                     /* ファイルポインタ */
