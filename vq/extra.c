    ...
    for (x = 0; x < x_block; x++) {
      ...
      for (i = 0; i < BLOCK_SIZE; i++) {
        for (j = 0; j < BLOCK_SIZE; j++) {
          vector[y][x][i * BLOCK_SIZE + j] = image1[ys + i][xs + j];
        }
      }
    }
  }
...
/* K-平均法によるクラスタリングを行う */
{
  ...
  while (!finish) {
    ...
    /* 各ブロックの所属クラスタ番号の初期化 */
    for (y = 0; y < y_block; y++)
      for (x = 0; x < x_block; x++) cluster[y][x] = -1;
    /* 各点の所属クラスタを決定する */
    for (y = 0; y < y_block; y++) {
      for (x = 0; x < x_block; x++) {
        /* 各クラスタ代表点との距離を求める */
      ...
      for (y = 0; y < y_block; y++) {
        for (x = 0; x < x_block; x++) {
          if (cluster[y][x] == i) {
            ...
            for (j = 0; j < DIMENSION; j++) sum[j] = sum[j] + vector[y][x][j];
          }
        }
      }
      for (j = 0; j < DIMENSION; j++) {
        main_vector[i][j] = (unsigned char)(sum[j] / sum_number);
      }
    }
    ...
    for (i = 0; i < K_number; i++) {
      ...
      for (j = 0; j < DIMENSION; j++) {
        difference =
            difference + abs(init_main_vector[i][j] - main_vector[i][j]);
      }
      ...
      for (y = 0; y < y_block; y++) {
        for (x = 0; x < x_block; x++) {
          ...
          for (i = 0; i < BLOCK_SIZE; i++) {
            for (j = 0; j < BLOCK_SIZE; j++) {
              image2[ys + i][xs + j] =
                  main_vector[cluster[y][x]][i * BLOCK_SIZE + j];
            }
          }
        }
      }
  ...
  coded_data_size = x_block * y_block * j
                    /* 代表ベクトル */
                    + BLOCK_SIZE * BLOCK_SIZE * K_number * 8;
  /* ルックアップテーブル */
  printf("圧縮データ（画像部分）のデータ量 = %d\n", coded_data_size);
  compression_rate =
      (double)coded_data_size / (double)original_data_size * 100.0;
  printf("圧縮率：%f [％]\n", compression_rate);

// bunsi
  for(int i = 0; i < y_size1; ++i){
    for(int j = 0; j < x_size1; ++j){
      bunsi += pow((double)image1[i][j], 2);
    }
  }
...
// bunbo
  for (int i = 0; i < y_size2; ++i) {
    for (int j = 0; j < x_size2; ++j) {
      bunbo += pow( (image1[i][j] - image2[i][j]), 2);
    }
  }
}
...
int main() {
  ...
  snr = 10 * log10(bunsi / bunbo);
  printf("snr = %lf[dB]", snr);
